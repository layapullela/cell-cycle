# this code is directly from piq library to compute the iw-ssim loss. filter functions are copied below.
...
# a couple of changes
# 1. range: we use data_range = 4, but must shift output to [0, 4] via (x + 2).clamp(0, 4)
# 2. reduce the pyramid depth from 5 to 3 since N = 64 supports at most 3 levels
# 3. IW-ssim written for single channel images, our images are 5 channel. we have to change the code to handle this.
# 4. some hic matrices are close to 0, so we add a small epsilon to the diagonal of the covariance matrix to avoid division by zero.
...
def information_weighted_ssim(x: torch.Tensor, y: torch.Tensor, data_range: Union[int, float] = 1.,
    ...
    if scale_weights is None:
        ...
        scale_weights = scale_weights.to(dtype=x.dtype, device=x.device)
...
def _pyr_step(x: torch.Tensor, kernel: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    """
    n_channels = x.size(1)
    kernel_h = kernel.expand(n_channels, 1, 1, -1)
    kernel_v = kernel.transpose(-2, -1).expand(n_channels, 1, -1, 1)
    ...
    if x.size(-1) > 1:
        ...
        lo_x = F.conv2d(input=lo_x, weight=kernel_h, groups=n_channels, padding=0)[:, :, :, ::2]
    if x.size(-2) > 1:
        lo_x = F.pad(lo_x, pad=[0, 0, up_pad, down_pad], mode='reflect')
        lo_x = F.conv2d(input=lo_x, weight=kernel_v, groups=n_channels, padding=0)[:, :, ::2, :]
    ...
    if x.size(-1) > 1:
        ...
        upsampling_kernel = upsampling_kernel.expand(n_channels, 1, 1, 2)
        hi_x = F.conv_transpose2d(
            input=hi_x, weight=upsampling_kernel, stride=(1, 2), padding=0, groups=n_channels,
        )
        hi_x = F.pad(hi_x, pad=[up_pad, down_pad, 0, 0], mode='reflect')
        hi_x = F.conv2d(input=hi_x, weight=kernel_h, groups=n_channels, padding=0)[:, :, :, :x.size(-1)]
    if x.size(-2) > 1:
        upsampling_kernel = torch.tensor([[[[1.], [0.]]]], dtype=x.dtype, device=x.device)
        upsampling_kernel = upsampling_kernel.expand(n_channels, 1, 2, 1)
        hi_x = F.conv_transpose2d(
            input=hi_x, weight=upsampling_kernel, stride=(2, 1), padding=0, groups=n_channels,
        )
...
def _information_content(x: torch.Tensor, y: torch.Tensor, y_parent: torch.Tensor = None,
    ...
    C_u_inv = torch.linalg.solve(
        C_u + torch.eye(C_u.size(-1), dtype=C_u.dtype, device=C_u.device) * 1e-6,
        torch.eye(C_u.size(-1), dtype=C_u.dtype, device=C_u.device).expand_as(C_u),
    )
...
def _image_enlarge(x: torch.Tensor) -> torch.Tensor:
    ...
    t2 = torch.zeros([x.size(0), x.size(1), 4 * x.size(-2) - 1, 4 * x.size(-1) - 1], device=x.device, dtype=x.dtype)
    t2[:, :, 1: -1, 1:-1] = t1
    t2[:, :, 0, :] = 2 * t2[:, :, 1, :] - t2[:, :, 2, :]
    t2[:, :, -1, :] = 2 * t2[:, :, -2, :] - t2[:, :, -3, :]
    t2[:, :, :, 0] = 2 * t2[:, :, :, 1] - t2[:, :, :, 2]
